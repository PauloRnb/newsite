    ...
    <nav>
      <Dialog.Root open={open} onOpenChange={setOpen}>
        ...
        <Dialog.Portal>
          <Dialog.Overlay
            forceMount
            className={`z-50 fixed inset-0 backdrop-blur-[2px] transition-opacity duration-300 ${
              open ? "animate-fadeIn" : "animate-fadeOut"
            }`}
          />
          <Dialog.Content
            forceMount
            className={`z-50 fixed top-0 right-0 h-full inset-y-0 border-l sm:max-w-sm md:max-w-[615px] w-full overflow-y-auto dark:bg-zinc-800 bg-white shadow-lg select-none flex flex-col transition-transform duration-300 ${
